 ...
 * Activation Module
 * 
 *  alt_mod.cxx - finds an activation in a time/voltage array
 *
 */
...
//    cerr << "Kelly's APD/alternans data: "<< endl;
    ...
    for (i = 0; i < nbeats+2; i++){
        ...
        for (j = i*bcl_ndata+1; j < (i+1)*bcl_ndata; j++){
            ...
            // Make sure Vm crosses -40 mV
            if (block && vdata[j] >= -0.040) {
                block = false;
                status = -1;
            }
            // Find peak
            ...
            if (!block && status == -2){
                ...
                else if (j > maxvtime+1 && j < maxvtime+100){
